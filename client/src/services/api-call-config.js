...
class APICallConfig {
    static callAPIService = (APIName, parameter) => {
        //console.log('api service', APIName);
        const APIInfo = endPoint[APIName];
        return APICallConfig[APIInfo.type](APIInfo, parameter)
        .then(response => {
            return response;
        });
    }
    ...
    static get = (APIInfo, parameter) => {
        return http.get((parameter)? this.generateAPIParameterWithURL(APIInfo, parameter)
                    : APIInfo.url, headerOptions[APIInfo.headerType])
        .then(response => {
    ...
    static post = (APIInfo, parameter) => {
        return http.post(APIInfo.url, parameter, headerOptions[APIInfo.headerType])
        .then(response => {
            return response.data;
        }).catch(error => {
            return Promise.reject(error);
    ...
    static delete = (APIInfo, pathParameters, parameterBody) => {
        ...
        return http.delete(APIInfo.url + '/' + resPathParamURL, parameterBody, headerOptions[APIInfo.headerType])
        .then(response => {
            return response.data;
        }).catch(error => {
            return Promise.reject(error);
